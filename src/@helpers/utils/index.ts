...
const pipeObjectRender = <T = any>(obj: T): {
...
const checkAndRenderMessErr = (fieldCheck: any) => fieldCheck ? 'flex' : 'none';

const replaceNumberal = (number: string) => parseInt(number.replace(/,/g, ''));

const parseEnum = (data: object) => Object.keys(data).map((key) => ({ ...data[key] }));

const mergeDate = (date: Date, time: Date) => new Date(moment(`${Utils.pipeDate(date, 'YYYY-MM-DD')} ${Utils.pipeDate(time, 'HH:mm:ss')}`, 'YYYY-MM-DD HH:mm:ss').format());

const capitalizeFirstLetter = (string: string) => {
    return string.charAt(0) + string.slice(1).toLowerCase();
}

const subtractDate = (date: Date, number: number) => new Date(date.setDate(date.getDate() - number));
